    ...
    Variants {
        ...
        PanelWindow {
            ...
            property var filteredApps: {
                const stxt = search ? search.text.toLowerCase().trim() : "";
                ...
                for (let i = 0; i < all.length; ++i) {
                    ...
                    if (name === stxt) {
                        score = 0; // Exact match
                    } else if (name.startsWith(stxt)) {
                        score = 1; // Starts with query (e.g., "discord" -> Discord)
                    } else if (name.split(/[\s\-_]+/).some(w => w.startsWith(stxt))) {
                        score = 2; // Word boundary match
                    } else if (name.includes(stxt)) {
                        ...
                        for (let si = 0; si < stxt.length; ++si) {
                            const sc = stxt[si];
                            while (ni < name.length && name[ni] !== sc) {
                                ni++;
                            }
                            if (ni >= name.length) {
                                matched = false;
                    ...
                    if (score >= 0) {
                        scored.push({ app: app, score: score, name: app.name || "" });
                    }
                }
